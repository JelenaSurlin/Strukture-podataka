	...
	if (root != NULL)
	{
		...
		while (head != NULL) {
			...
			if (head->listElement->left != NULL)
				head = addToTheEndOfList(head, head->listElement->left);
			if (head->listElement->right != NULL)
				head = addToTheEndOfList(head, head->listElement->right);
			head = head->next;
		}
